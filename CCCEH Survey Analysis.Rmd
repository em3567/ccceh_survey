...
#This is not coding missing data as NA so you cannot remove the missing values -- affecting proprtions
Q26 <- all_practice_data %>% select(ID, X1_Q26, X2_Q26, X3_Q26, X4_Q26, X5_Q26)
#is.na(Q26$X3_Q26) Returning false -- no NAs
Q26 <- Q26 %>% pivot_longer(!ID, names_to = "Child", values_to = "Duration")

Q26 %>% count(Duration)
Q26 <- Q26 %>% filter(!is.na(Duration)) %>% count(Duration) %>% mutate(prop = n/sum(n))
view(Q26)
```

###All Chlildren
```{r}
#We will need to recode all Prefer not ot answer/do not know to NA
...
#This is not working right now because missing values are not NA/Prefer not to answer/do not know but once they are this will be correct. 
all_practice_data %>%
  select(-Q40=="NA") %>%
  count(Q40) %>%
  mutate(prop = n/sum(n))
...
```{r}
all_practice_data2 <- all_practice_data
missing_values <- c("")
all_practice_data2 %>%
  replace_with_na_all(condition = ~.x %in% missing_values)
view(all_practice_data2)
#This only worked for questions that return a number

#all_practice_data2 <- all_practice_data2 %>%
#  mutate(across(where(is.character), ~na.if(., "")))



#all_practice_data2 %>% replace_with_na_all(condition = ~.x == "")
#view(all_practice_data2)
```
