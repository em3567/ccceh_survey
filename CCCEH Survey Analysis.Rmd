...
```{r}
#Race Ethnicity
Q6 <- all_practice_data %>% select(ID, Q6)
Q6 <- Q6 %>% pivot_longer(!ID, names_to = "parent", values_to = "parent_race_eth")
...
#income
Q8 <- all_practice_data %>% select(ID, Q8)
Q8 <- Q8 %>% pivot_longer(!ID, names_to = "parent", values_to = "income")
```
...
#By child gender
Q30_gender <- Q13 %>% select(child_gender) %>% cbind(Q30) %>% select(ID, child, outside, child_gender)
# view(Q30_gender)
...
#BY child race
#By child gender
Q30_race <- Q14 %>% select(child_race_eth) %>% cbind(Q30) %>% select(ID, child, outside, child_race_eth)
# view(Q30_gender)
...
Q31_income <- Q8 %>% select(income, parent) %>% cbind(Q31) %>% select(ID, child, rating, income, parent)
...
Q39 <- all_practice_data %>% select(ID, Q39)
Q39 <- Q39 %>% pivot_longer(!ID, names_to = "parent", values_to = "ingredients")
Q39 %>% filter(!is.na(ingredients)) %>% count(ingredients) %>% mutate(prop = n/sum(n))
...
Q39_demo <- Q6 %>% select(parent_race_eth) %>% cbind(Q39) %>% select(ID, parent, ingredients, parent_race_eth)
# view(Q39_demo)
