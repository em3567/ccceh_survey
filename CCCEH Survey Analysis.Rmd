...
Q12 <- Q12_2 %>% select(age_cat) %>% cbind(Q12) %>% select(ID, child, age, age_cat)

# Proportions for each individual age
Q12 %>% drop_na() %>% count(age) %>% mutate(prop = n/sum(n))
...
```{r}
# Q14 What race identity best describes Child X?
...
>>>>>>> 3d77ff6ecc0e1f493c275c4288b0b27ecde686ea

Q14 <- all_practice_data %>% select(ID, X1_Q14, X2_Q14, X3_Q14, X4_Q14, X5_Q14)
#view(Q13)
Q14 <- Q14 %>% pivot_longer(!ID, names_to = "child", values_to = "race_eth")
#view(Q13)
...
# Rating by child gender 
Q13_2 <- Q13 %>% select(gender)
Q31_3 <- Q31 %>% select(rating)
Q31_Q13 <- cbind(Q13_2, Q31_3)
view(Q31_Q13)
Q31_Q13 %>%
  filter(!is.na(gender)) %>%
  filter(!is.na(rating)) %>%
...
Q31_Q13 %>% drop_na() %>%
  group_by(fct_relevel(gender, "Male", "Female")) %>%
  summarise(mean_rating = mean(rating))
#Results 
#Gender   Rate
#Male	    3.0	
#Female 	6.5
#Women are more likely to use products for beautification. Men are more likely to use products for play. 



# Rating by child race
#I'm honestly not sure this breakdown even matters. I think having the overall mean by race category is more important. 
Q14_2 <- Q14 %>% select(race)
Q31_3 <- Q31 %>% select(rating)
Q31_Q14 <- cbind(Q14_2, Q31_3)
view(Q31_Q14)
Q31_Q14 %>%
  filter(!is.na(race)) %>%
  filter(!is.na(rating)) %>%
...
#Results
#Score Race    n %
#1	Black or African American,White	  2	0.5	
#1	East Asian	                      2	0.5	
#2	Black or African American,White	  4	1.0	
#5	Black or African American,White	  6	1.0	
#7	Black or African American,White	  2	1.0	
#8	American Indian or Alaska Native	2	0.5	
#8	East Asian	                      2	0.5	
#10	Black or African American,White	  2	0.5	
#10	East Asian	                      2	0.5	

# Mean rating by Gender
Q31_Q14 %>% drop_na() %>%
  group_by(fct_relevel(race)) %>%
  summarise(mean_rating = mean(rating))
#Results 
#Race                             Rate
...
#East Asian	                      6.333333
#American Indians are most likely to use products for beautification. Black and White Americans are most likely to use products for play. 
=======
#Results
#0-3	  9.333333	
#4-6	  6.250000	
...
Q26_Q12 <- Q12 %>% select(age_cat) %>% cbind(Q26) %>% select(ID, child, duration, age_cat)
# view(Q26_Q12)
...
Q26_Q13 <- Q13 %>% select(gender) %>% cbind(Q26) %>% select(ID, child, duration, gender)
#view(Q26_Q13)
...
Q26_Q14 <- Q14 %>% select(race_eth) %>% cbind(Q26) %>% select(ID, child, duration, race_eth)
#view(Q26_Q14)
